% [psf psfTwoPhotonSwiped psfTwoPhotonCylindricalLens]=calcLightSheetPsf(xRange,yRange,zRange,tilt,excitation,modulation,refractiveIndexOfSample)
...
%     modulation.beta=1;
%     
%     lightSheet=calcLightSheetPsf(xRange,yRange,zRange,0,excitation,modulation,1.4);
...
%
function [psf psfTwoPhotonSwiped psfTwoPhotonCylindricalLens]=calcLightSheetPsf(xRange,yRange,zRange,tilt,excitation,modulation,refractiveIndexOfSample)
    ...
    %pupilFunctor=@(U,V) crop(U,V).*(sqrt(U.^2+V.^2)>=(1-modulation.beta)).*exp(2i*pi*(modulation.alpha*V.^3 + (tilt-modulation.alpha*3/5)*V));
